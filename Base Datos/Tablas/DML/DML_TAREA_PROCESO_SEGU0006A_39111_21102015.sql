...
INSERT INTO AVALUOS.TAREA_PROCESO (CODIGO_TAREA, TRANSACCION, INSTRUCCION, PROCESO, 
                           ID_BLOQUE, TRANSACCION_ALTERNA, JOIN, REFERENCES, 
                           TAB_TITLE, VALIDATE_FORM, FILE_OUTPUT, INITIAL_PARAMS, 
                           TITLE_DATA, DATASOURCE, NUM_COLUMN, INITIAL_STATED)
VALUES ('SEGU0006A', 'baseweb.ServPrograma', 'SELECT LPAD('' -> '', LEVEL*5)||C.MENU AS MENU,
       C.CODIGO_TAREA,
       C.DESCRIPCION_TAREA,
       C.PADRE,
       C.NOMBRE_PADRE,
...
CONNECT BY PRIOR C.CODIGO_TAREA = C.PADRE', 
        '', 0, '', '', '', '', '', 'sendExcel.jsp', 
        '', '', '', 16, 0);
