...
@NamedQueries({@NamedQuery(name = "getValoresDominioAux", query = "select d from Dominios d WHERE d.rvDomain = :nombreDominio order by d.rvLowValue")
			  ,@NamedQuery(name = "getValoresDominioAuxByAbbreviation", query = "select d from Dominios d WHERE d.rvDomain = :nombreDominio AND d.rvAbbreviation = :abbreviation order by d.rvMeaning")	})
@org.eclipse.persistence.annotations.Cache(type = SOFT, alwaysRefresh = false, refreshOnlyIfNewer = true)
public class DominiosAux implements Serializable {
	private static final long serialVersionUID = 1L;

	@Id
	@SequenceGenerator(name="CG_REF_CODES_IDCGREFCODES_GENERATOR_AUX", sequenceName="SEQ_CG_REF_CODES", allocationSize=1)
	...
	public DominiosAux(String rvDomain, String rvLowValue, String rvMeaning) {
